...
func (cfg *apiConfig) handlerCreateMusicMeta(w http.ResponseWriter, r *http.Request) {
	...
	musicMeta, err := cfg.db.CreateMusicMeta(r.Context(), database.CreateMusicMetaParams{
		ID:          uuid.New().String(),
		UserID:      user.ID,
	...
	httpapi.RespondWithJSON(w, http.StatusOK, musicMeta)
